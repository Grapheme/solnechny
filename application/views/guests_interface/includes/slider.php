<div class="nav index">
	<div class="nav-container">
		<nav>
			<div class="nav-big">
				<ul>
					<li><a class="big-option" href="<?=site_url('about');?>">О КЛУБЕ</a></li>
					<li><a class="big-option" href="<?=site_url('pool');?>">БАССЕЙН</a></li>
					<li><a class="big-option" href="<?=site_url('restaurant');?>">РЕСТОРАН</a></li>
					<li><a class="big-option" href="<?=site_url('child');?>">ДЕТСКИЙ КЛУБ</a></li>
					<li><a class="big-option" href="<?=site_url('afisha');?>">АФИША</a></li>
					<li><a class="big-option" href="<?=site_url('contacts');?>">КОНТАКТЫ</a></li>
					<li><a class="big-option res big" href="<?=site_url('bron');?>"><img class="res-img" src="<?=site_url('img/bron.png');?>">БРОНИРОВАНИЕ</a></li>
				</ul>
			</div>
			<div class="nav-med">
				<div class="med-mar">
					<ul>
						<li><a class="med-option def-index" href="#">МЕНЮ</a></li>
						<li><a class="med-option res" href="#"><img class="res-img" src="<?=site_url('img/bron.png');?>">БРОНИРОВАНИЕ</a></li>
					</ul>
				</div>
			</div>
			<div class="nav-small">
				<ul>
					<li><a class="small-option def" href="#">МЕНЮ</a></li>
					<li><a class="small-option res" href="#">БРОНИРОВАНИЕ</a></li>
				</ul>
			</div>
		</nav>
	</div>
</div>
